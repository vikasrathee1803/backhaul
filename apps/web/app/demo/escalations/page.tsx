...
export default function EscalationsPage() {
  ...
  const [selectedDisposition, setSelectedDisposition] = useState<Record<string, string>>({});

  const escalated = DEMO_RETURNS.filter((r) => r.status === "escalated" && !dismissed.has(r.id));

  function handleDismiss(id: string) {
    setDismissed((prev) => new Set(Array.from(prev).concat(id)));
  }

  function handleSelectDisposition(returnId: string, disposition: string) {
    setSelectedDisposition((prev) => ({ ...prev, [returnId]: disposition }));
  }
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 28px 40px" }}>
          ...
          {escalated.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {escalated.map((r) => {
                const activeDispo = selectedDisposition[r.id];
                return (
                  <div
                    key={r.id}
                    className="card"
                        ...
                        <div style={{ marginTop: 12 }}>
                          ...
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            {["repair", "replace", "refurbish"].map((d) => {
                              const isActive = activeDispo === d;
                              return (
                                <button
                                  key={d}
                                  onClick={() => handleSelectDisposition(r.id, d)}
                                  style={{
                          ...
                          {activeDispo && (
                            <div style={{ marginTop: 8, fontSize: 12, color: "var(--success)" }}>
                              Disposition selected: <strong>{activeDispo}</strong> — submit via Review page
                            </div>
                          )}
                        </div>
                      </div>
